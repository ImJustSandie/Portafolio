...
export const ConfidantBar: React.FC<ConfidantBarProps> = ({ title, rank, percentage, description, tags }) => {
  ...
  return (
    <div className="group cursor-pointer flex flex-col overflow-x-clip" onClick={() => setIsExpanded(!isExpanded)}>
      <div className="flex justify-between items-end mb-1 gap-2">
        <span className="font-display text-sm sm:text-lg md:text-2xl italic group-hover:text-p5-red transition-colors truncate min-w-0">
      ...
      {tags && tags.length > 0 && (
        <div className="flex flex-wrap gap-1 md:gap-2 mt-1 md:mt-2 overflow-x-clip">
          {tags.map((tag) => (
            <span
              key={tag.icon}
              className="inline-flex items-center gap-1 bg-p5-black text-p5-white font-mono text-[10px] md:text-xs tracking-wider px-1.5 md:px-2 py-0.5 md:py-1 border border-p5-white/30 -skew-x-[6deg] hover:bg-p5-red hover:border-p5-red transition-colors group/tag shrink-0 max-w-full overflow-x-clip"
            >
              <span className="inline-flex skew-x-[6deg] items-center gap-1 truncate max-w-full">
                {tag.icon && (
                  <span className="w-2.5 h-2.5 md:w-3.5 md:h-3.5 flex items-center justify-center text-p5-white/80 group-hover/tag:text-p5-white transition-colors shrink-0">
      ...
      >
        <div className="bg-p5-black text-p5-white p-2 md:p-4 border-2 md:border-4 border-p5-white relative shadow-hard flex flex-col transition-transform hover:-translate-y-1">
          <div
            className="absolute inset-0 opacity-10 pointer-events-none"
            style={{ backgroundImage: 'radial-gradient(#F4F4F4 1px, transparent 1px)', backgroundSize: '8px 8px' }}
          ></div>
          <div className="absolute -top-2 md:-top-3 -left-1 md:-left-2 bg-p5-red text-p5-white font-mono text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 border-2 border-p5-white -skew-x-[6deg] shadow-sm z-10">
          ...
          <p className="font-body text-xs md:text-sm opacity-90 leading-relaxed relative z-10 mt-0.5 md:mt-1 pl-1.5 md:pl-2 border-l-2 border-p5-red">
